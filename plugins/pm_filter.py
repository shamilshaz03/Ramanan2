...
async def next_page(bot, query):
    ...
    btn.insert(0, [
        InlineKeyboardButton("🗄 ꜱᴇɴᴅ ꜰɪʟᴇꜱ", callback_data=f"send_fall#files#{offset}#{req}"),
...
async def language_check(bot, query):
    ...
    if offset != "":
        ...
        try:await query.edit_message_reply_markup(
            ...
            pass
        await query.answer()
    else:return await query.answer(f"Sᴏʀʀʏ, Nᴏ ғɪʟᴇs ғᴏᴜɴᴅ ғᴏʀ ʏᴏᴜʀ ᴏ̨ᴜᴇʀʏ {movie}.", show_alert=True)
            ...
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                if clicked == typed:
                    if IS_VERIFY and not await check_verification(client, query.from_user.id):
                        btn = [[
                            InlineKeyboardButton("⋆ ᴠᴇʀɪꜰʏ", url=await get_token(client, query.from_user.id, f"https://telegram.me/{temp.U_NAME}?start=", file_id)),
